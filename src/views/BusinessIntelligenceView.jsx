...
import { money, computeProductInsights, marginTone } from "../lib/utils.js";
...
export default function BusinessIntelligenceView({ orders, products }) {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h2 className="text-xl font-bold text-text-primary" style={{ fontFamily: displayFont }}>Business Intelligence</h2>
        <p className="text-sm text-gray-400">Profit, best-sellers, and dead stock — computed from your live orders and inventory.</p>
      </div>
      ...
      <Card noPad>
        <div className="flex items-center justify-between px-5 pt-5 mb-3">
          <h3 className="font-bold text-text-primary text-sm" style={{ fontFamily: displayFont }}>Best-selling products</h3>
          ...
          <div className="px-5 pb-5 space-y-3">
            {bestSellers.map((p, i) => (
              <div key={p.id} className="flex items-center gap-3">
                <div className="w-6 h-6 rounded-full bg-indigo-900/30 text-indigo-400 text-[11px] font-bold flex items-center justify-center shrink-0">
                  {i + 1}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-xs font-semibold text-text-primary truncate">{p.name}</span>
                    <span className="text-xs font-semibold text-gray-400 shrink-0 ml-2">{p.unitsSold} sold · {money(p.revenue)}</span>
                  </div>
                  <div className="h-1.5 rounded-full bg-slate-800 overflow-hidden">
                    <div className="h-full bg-indigo-400 rounded-full" style={{ width: `${Math.max(4, (p.unitsSold / topSellerUnits) * 100)}%` }} />
                  </div>
                </div>
              </div>
            ))}
      ...
      <Card noPad>
        <h3 className="font-bold text-text-primary text-sm px-5 pt-5 mb-1" style={{ fontFamily: displayFont }}>Profit &amp; margin per product</h3>
            ...
            <tbody>
              {byMargin.map((p) => (
                <tr key={p.id} className="border-t border-white/5">
                  <td className="px-5 py-3 font-semibold text-text-primary">{p.name}</td>
                  <td className="px-5 py-3 text-gray-400">{money(p.price)}</td>
                  <td className="px-5 py-3 text-gray-500">{money(p.cost)}</td>
                  <td className="px-5 py-3">
                    <Badge tone={marginTone(p.marginPct)}>{money(p.marginRs)} · {p.marginPct.toFixed(0)}%</Badge>
                  </td>
                  <td className="px-5 py-3 text-gray-400">{p.unitsSold}</td>
                  <td className="px-5 py-3 font-medium text-text-primary">{money(p.profitContribution)}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <Card noPad>
        <div className="flex items-center justify-between px-5 pt-5 mb-1">
          <h3 className="font-bold text-text-primary text-sm" style={{ fontFamily: displayFont }}>Dead stock report</h3>
              ...
              <tbody>
                {deadStock.map((p) => (
                  <tr key={p.id} className="border-t border-white/5">
                    ...
                    <td className="px-5 py-3 text-gray-400">{p.stock} units</td>
