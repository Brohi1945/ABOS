  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <h2 className="text-xl font-bold text-[#E8E9ED]" style={{ fontFamily: displayFont }}>Inventory</h2>
                  ...
                  <tr key={p.id} className="border-t border-[rgba(255,255,255,0.06)] hover:bg-white/5 transition">
                    ...
                    <td className="px-5 py-3.5">
                      <div className="flex items-center justify-end gap-1">
                        <button onClick={() => setModal({ mode: "edit", product: p })} className="w-8 h-8 rounded-lg hover:bg-white/5 flex items-center justify-center text-[#8B8F9C]">
                          <Pencil size={14} />
                        </button>
                        <button onClick={() => setConfirmDelete(p)} className="w-8 h-8 rounded-lg hover:bg-red-500/10 flex items-center justify-center text-[#8B8F9C] hover:text-red-400">
                          <Trash2 size={14} />
                        </button>
                      </div>
                    </td>
        ...
        {confirmDelete && (
          <div>
            <p className="text-sm text-[#C7C9D1] mb-5">
              Remove <span className="font-semibold text-[#E8E9ED]">{confirmDelete.name}</span> from inventory? This can't be undone.
            </p>
