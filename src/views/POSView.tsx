...
import { ShoppingCart, Barcode, Plus, X, Minus, CreditCard, Printer, Search } from "lucide-react";
import { displayFont } from "../theme";
import { genId, money, matchesQuery, Product } from "../lib/utils";
import { Card, Badge, Button, Modal, EmptyState, inputCls } from "../components/ui";
...
export default function POSView({ products, onCheckout }: POSViewProps) {
  ...
  const visibleProducts = products.filter((p) => matchesQuery(productQuery, p.name, p.category, p.barcode));
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 space-y-4">
          <Card>
            ...
            {scanMsg && (
              <div className={`mt-2.5 text-xs font-semibold px-3 py-2 rounded-lg ${scanMsg.ok ? "bg-green-500/10 text-green-400" : "bg-red-500/10 text-red-400"}`}>
                {scanMsg.text}
              </div>
            )}
          </Card>
          ...
          <Card noPad>
            <div className="flex items-center justify-between px-5 pt-5 mb-3 gap-3">
              <h3 className="font-bold text-fg text-sm shrink-0" style={{ fontFamily: displayFont }}>Quick-add products</h3>
            ...
            {visibleProducts.length === 0 ? (
              <div className="px-5 pb-5">
                <EmptyState icon={Search} title="No products match" note="Try a different name or category." />
              </div>
            ) : (
