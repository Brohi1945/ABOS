...
import { computeWeeklyTrend, computeProductInsights, Order, Product, Customer } from "../lib/utils";
import { useVoiceInput } from "../lib/useVoiceInput";
import { useVoiceOutput } from "../lib/useVoiceOutput";
import { detectVoiceToggleCommand, detectNavigationCommand, detectThemeCommand } from "../lib/voiceCommands";
...
export default function AssistantView({
  ...
  const suggestions = ["Business health report do", "Sabse zyada profit konsa product de raha hay", "Kaunsa customer risk pe hay", "Colorful theme laga do"];
  ...
  const send = async (text?: string) => {
    ...
    // "dark mode laga do" / "colorful theme kar do" / "light theme on karo"
    // — same reasoning as the voice toggle above: the LLM has no way to
    // actually flip our CSS theme classes, so we intercept and switch it
    // directly via ThemeProvider.setTheme.
