...
import { Megaphone, Plus, Send, Mail, MessageSquare, Smartphone, Eye, MousePointerClick, Search, X } from "lucide-react";
import { displayFont } from "../theme";
import { Card, Badge, StatCard, Button, Modal, Field, inputCls, EmptyState } from "../components/ui";
import { matchesQuery, Campaign } from "../lib/utils";
...
export default function MarketingView({ campaigns, onAdd }: MarketingViewProps) {
  ...
  const filteredCampaigns = campaigns
    .filter((c) => (channelFilter === "all" ? true : c.channel === channelFilter))
    .filter((c) => matchesQuery(query, c.name, c.channel, c.status));
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <div className="flex gap-1.5 overflow-x-auto">
          {["all", ...Object.keys(CHANNEL_META)].map((ch) => (
            <button
              key={ch}
              onClick={() => setChannelFilter(ch)}
              className={`text-xs font-semibold px-3 py-2 rounded-xl whitespace-nowrap transition ${
                channelFilter === ch ? "bg-brand text-white" : "bg-app border text-muted"
              }`}
            >
      ...
      {filteredCampaigns.length === 0 ? (
        <EmptyState
          ...
          note={campaigns.length === 0 ? "Create your first campaign to start reaching customers." : "Try a different search or channel."}
        />
      ) : (
