...
import { money, computeCustomerStats, Customer, Order } from "../lib/utils";
...
function CustomerForm({ initial = null, onSave, onCancel }: CustomerFormProps) {
  const [form, setForm] = useState<any>(initial || { name: "", phone: "", email: "", address: "" });
  ...
  return (
    <div>
      ...
      <Field label="Address (optional)">
        <input value={form.address || ""} onChange={set("address")} className={inputCls} placeholder="e.g. House 12, Street 4, Gulshan, Karachi" />
      </Field>
      {form.address && <AddressMap address={form.address} height={130} />}
...
export default function CustomersView({ customers, orders, onAdd }: CustomersViewProps) {
        ...
        {selected && (() => {
          ...
          <div className="space-y-5">
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 rounded-full bg-indigo-500/10 text-indigo-400 flex items-center justify-center font-bold shrink-0">
            ...
            {selected.address && (
              <div>
                <div className="text-xs font-semibold text-muted mb-2">Address</div>
                <AddressMap address={selected.address} />
              </div>
            )}
