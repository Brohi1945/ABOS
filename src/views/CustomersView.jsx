...
import { money, computeCustomerStats } from "../lib/utils.js";
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <h2 className="text-xl font-bold text-text-primary" style={{ fontFamily: displayFont }}>Customers</h2>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.map((c) => {
                ...
                return (
                <tr key={c.id} onClick={() => setSelected(c)} className="border-t border-white/5 hover:bg-white/5 transition cursor-pointer">
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-2.5">
                      <div className="w-8 h-8 rounded-full bg-indigo-900/30 text-indigo-400 flex items-center justify-center text-xs font-bold shrink-0">
                  ...
                  <td className="px-5 py-3.5 text-gray-300">{stats.orders}</td>
                  <td className="px-5 py-3.5 text-text-primary font-medium">{money(stats.spent)}</td>
                  <td className="px-5 py-3.5 text-gray-500 text-xs">{stats.lastOrder}</td>
                  <td className="px-5 py-3.5 text-right"><ChevronRight size={15} className="text-gray-600" /></td>
                </tr>
                );
              })}
            </tbody>
        ...
        {selected && (() => {
          ...
          <div className="space-y-5">
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 rounded-full bg-indigo-900/30 text-indigo-400 flex items-center justify-center font-bold shrink-0">
                {selected.name.split(" ").map((n) => n[0]).join("").slice(0, 2)}
              </div>
              <div>
                <div className="font-semibold text-text-primary text-sm">{selected.name}</div>
                <div className="text-xs text-gray-400">{selected.phone} · {selected.email}</div>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="rounded-xl bg-slate-800 p-3">
                <div className="text-[10px] text-gray-400 font-medium mb-1">Total orders</div>
                <div className="text-lg font-bold text-text-primary">{stats.orders}</div>
              </div>
              <div className="rounded-xl bg-slate-800 p-3">
                <div className="text-[10px] text-gray-400 font-medium mb-1">Total spent</div>
                <div className="text-lg font-bold text-text-primary">{money(stats.spent)}</div>
              </div>
            </div>
            <div>
              <div className="text-xs font-semibold text-gray-400 mb-2">Order history</div>
              <div className="space-y-2">
                {orders.filter((o) => o.customer === selected.name).map((o) => (
                  <div key={o.id} className="flex items-center justify-between bg-slate-800 rounded-lg px-3 py-2.5 text-sm">
                    <div>
                      <div className="font-semibold text-text-primary">{o.id}</div>
                      <div className="text-[11px] text-gray-500">{o.date}</div>
                    </div>
                    <div className="text-right">
                      <div className="font-semibold text-text-primary">{money(o.total)}</div>
                      <StatusBadge status={o.status} />
                    </div>
                  </div>
                ))}
                {orders.filter((o) => o.customer === selected.name).length === 0 && (
                  <div className="text-xs text-gray-500 text-center py-4">No orders yet.</div>
                )}
              </div>
            </div>
          </div>
