...
import { money, computeWeeklyTrend, Order, Product } from "../lib/utils";
...
export default function AccountingView({ orders, products }: AccountingViewProps) {
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-xl font-bold text-fg" style={{ fontFamily: displayFont }}>Accounting overview</h2>
      ...
      <Card>
        <h3 className="font-bold text-fg text-sm mb-3" style={{ fontFamily: displayFont }}>Revenue vs cost — daily</h3>
        <div className="h-64">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={weeklyTrend.map((d) => ({ ...d, cost: Math.round(d.sales * 0.68) }))}>
              ...
              <Tooltip
                ...
                contentStyle={{ background: "#111827", borderRadius: 12, border: "1px solid rgba(255, 255, 255, 0.08)", fontSize: 12 }}
                labelStyle={{ color: "#9CA3AF" }}
                itemStyle={{ color: "#E5E7EB" }}
              />
              {/* 👇 using CHART_ANIMATION spread */}
              <Bar dataKey="sales" fill="#6366F1" radius={[6, 6, 0, 0]} name="Revenue" {...CHART_ANIMATION} />
      ...
      <Card noPad>
        <h3 className="font-bold text-fg text-sm px-5 pt-5 mb-1" style={{ fontFamily: displayFont }}>Recent transactions</h3>
            ...
            <tbody>
              {orders.slice(0, 6).map((o) => (
                <tr key={o.id} className="border-t border">
                  ...
                  <td className="px-5 py-3 text-muted">Sale</td>
                  <td className="px-5 py-3 text-muted text-xs">{o.date}</td>
                </tr>
              ))}
            </tbody>
          </table>
