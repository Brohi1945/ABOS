...
import {
  Bot, Plus, X, Sparkles, Send, ShoppingBag, Minus, ArrowLeft, CheckCircle2, PackageCheck, Loader2, Mic, MicOff, Volume2, VolumeX,
} from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-app" style={{ fontFamily: bodyFont }}>
      ...
      <Modal open={checkoutOpen} onClose={() => setCheckoutOpen(false)} title="Checkout" width={420}>
        ...
        <Button className="w-full" size="lg" onClick={handlePlaceOrder} disabled={!form.name || !form.phone || !form.email.includes("@") || !form.address}>
          Place order
        </Button>
      </Modal>
