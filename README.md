...
│   │
│   ├── components/
│   │   ├── layout.tsx            # Sidebar, Topbar (incl. ThemeSwitcher, global search trigger)
│   │   ├── ui.tsx                # Reusable primitives: Button, Card, Badge, StatCard, Drawer, Modal, Field, EmptyState
│   │   ├── Skeleton.tsx          # Loading skeletons
│   │   ├── GlobalSearch.tsx      # Ctrl/Cmd+K command-palette search (orders/products/customers/sections)
│   │   ├── DateRangeFilter.tsx   # Calendar/date-range filter dropdown (Today/Week/Month/Custom)
│   │   └── AddressMap.tsx        # Key-less Google Maps embed for an address
│   │
│   ├── lib/
...
│   │   ├── useKeyboardShortcuts.ts # Ctrl/Cmd+K/B/N, Esc, 1-9 — app-wide keyboard shortcuts
...
## 14. Recent Updates — Global Search, Calendar Filters, Maps, Keyboard Shortcuts, TypeScript (July 2026)

This update fixed the topbar search (which was previously decorative/non-functional) and filled in missing search/filter coverage across the admin dashboard, plus added calendar date filtering, Google Maps address previews, keyboard shortcuts, and tightened up several `any[]` prop types.

### New files
- **`src/components/GlobalSearch.tsx`** — Command-palette style global search. Opens with `Ctrl/Cmd+K` or by clicking the topbar search field. Searches sidebar sections, orders (id/customer), products (name/barcode), and customers (name/phone) all at once; arrow keys + Enter to navigate, click also works. Jumps straight to the relevant section on selection.
- **`src/components/DateRangeFilter.tsx`** — Reusable calendar/date-range filter dropdown with presets (Today, This week, This month, Custom range with native date pickers). Used by Orders and Waitlist; safe to reuse in any future view (Accounting, Business Intelligence, etc).
- **`src/components/AddressMap.tsx`** — Google Maps embed for a given address. Uses the key-less `google.com/maps?...&output=embed` URL, so **no Google Maps API key is required**. Shows an embedded map plus an "open in Google Maps" link.
- **`src/lib/useKeyboardShortcuts.ts`** — App-wide keyboard shortcut hook:
  - `Ctrl/Cmd + K` → open global search
  - `Ctrl/Cmd + B` → jump to POS (quick new bill)
  - `Ctrl/Cmd + N` → jump to Inventory and auto-open "Add product"
  - `Esc` → close search / notifications panel
  - `1`–`9` → jump to the Nth sidebar section (ignored while typing in a field)

### Search/filter fixes by screen
| Screen | Before | After |
|---|---|---|
| Topbar (global) | Decorative input, did nothing | Opens `GlobalSearch` command palette (⌘K) |
| Orders | Status tabs only | + text search (order ID / customer) + `DateRangeFilter` |
| Waitlist | Status filter only | + text search (customer / product name) + `DateRangeFilter` (on `joined_at`) |
| POS | Exact barcode/ID scan only | + product-name search box above the quick-add grid |
| Marketing | No search/filter at all | + campaign search + channel filter chips |
| Inventory, Customers | Already working | Unchanged |

### Google Maps
- **`src/config/app.config.ts`** — added `STORE_ADDRESS` constant (edit this to your real store address).
- **`src/screens/Store.tsx`** — storefront now shows a "Visit our store" map card (uses `STORE_ADDRESS`) at the bottom of the Shop tab.
- **`src/views/CustomersView.tsx`** — customer form now has an optional `address` field with a live map preview while typing, and the customer detail drawer shows the map when an address is on file.
- **`src/screens/AdminApp.tsx`** — `handleAddCustomer` now persists the `address` field.

### `lib/utils.ts` — new helpers and type fixes
- `matchesQuery(query, ...fields)` — shared case-insensitive multi-field text match used by every search box above.
- `DateRange`, `DatePreset`, `resolvePresetRange()`, `isWithinDateRange()` — power `DateRangeFilter`.
- `mapsEmbedUrl()` / `mapsSearchUrl()` — build the key-less Google Maps URLs used by `AddressMap`.
- `Order` interface — added missing `phone?` and `address?` fields (they were already being set/read in `AdminApp.tsx`, `Store.tsx`, and `AssistantView.tsx`, just not declared on the type).
- `Product` interface — added missing `barcode?` field (same situation — used everywhere in Inventory/POS but not declared).
- `Customer` interface — added `address?` field.
- New `Campaign` and `WaitlistEntry` interfaces.

### TypeScript cleanup
Replaced `any[]` props with proper `Product[]` / `Order[]` / `Customer[]` / `Campaign[]` types (from `lib/utils.ts`) in: `AdminApp.tsx`, `DashboardView.tsx`, `OrdersView.tsx`, `WaitlistView.tsx`, `POSView.tsx`, `MarketingView.tsx`, `CustomersView.tsx`, `InventoryView.tsx`, `AccountingView.tsx`, `BusinessIntelligenceView.tsx`, `AssistantView.tsx`, `Store.tsx`. `AdminApp.tsx` also gained a proper `AdminAppProps` interface instead of untyped destructuring, and a few handler signatures (`handleUpdateStatus`, `handleEditProduct`, `handleDeleteProduct`, `updateProductField`) are now typed instead of implicit `any`.

**Not yet fully typed** (left as-is to keep this change focused): form objects passed to `onSave`/`onAdd` handlers (`CustomerForm`, `ProductForm`, `CampaignForm`) are still `any` — they come straight from raw `<input>` state, which is normal/idiomatic for this pattern. `AssistantView.tsx` and `Store.tsx` still have some untyped internals beyond their top-level props. Project-wide, `tsconfig.json` still has `"strict": false`, so none of this is enforced at build time (`vite build` doesn't type-check) — it's purely for editor autocomplete/safety.

### How to apply this update
Copy the files listed above into the matching paths in this repo (they replace the existing versions 1:1), then:
```
npm install
npm run dev
```
No new dependencies were added — everything above uses packages already in `package.json` (React, lucide-react) plus native browser APIs (`<input type="date">`, Google Maps embed iframe — no API key needed).

---

## 15. Recent Updates — SaaS-level Animation Polish (July 2026)

Follow-up pass on top of the existing Framer Motion foundation (`staggerContainer`, `fadeSlideUp`, `useCountUp`, chart/modal/drawer transitions) — added navigation, feedback, and theme micro-interactions. No new dependencies (all via `framer-motion`, already in `package.json`).

### Changed files
- **`src/App.tsx`** — top-level screen switch (Landing → Login → Admin → Store) now crossfades via `AnimatePresence mode="wait"` + `fadeSlideUp`, instead of snapping instantly.
- **`src/components/layout.tsx`** —
  - Sidebar active nav item now has a gliding pill indicator behind it (`layoutId="nav-active-pill"`, spring transition) instead of an instant background-color swap.
  - Notification bell now shakes + the badge pulses when `notifCount` *increases* (tracked via a `useRef` so it never fires on mount or when the count goes down).
- **`src/components/GlobalSearch.tsx`** (⌘K command palette) — backdrop now has a blur + fade-in, the palette itself scales in, and results stagger in one-by-one (`staggerContainer` + `fadeSlideUp` from `animations/variants.ts`) instead of appearing all at once.
- **`src/components/Skeleton.tsx`** — removed a redundant/conflicting double-animation (Tailwind's `animate-pulse` was fighting the custom gradient keyframe); now a single clean diagonal light-sweep shimmer.
- **`src/index.css`** — theme switch (light/dark/colorful) now crossfades `background-color`/`border-color`/`color` across all elements (was previously only `body`, so cards/borders used to "snap" while the background faded).
- **`src/theme/tokens.css`** — the Colorful theme's aurora background gradients now slowly drift (`aurora-drift`, 24s loop, via animated CSS `@property` custom properties) instead of sitting static.

### Not done yet (bigger/riskier, worth discussing before building)
- Shared `layoutId` expansion from a stat card into a detail drawer — needs careful layout planning per view.
- Toast swipe-to-dismiss — `react-hot-toast` doesn't support this natively; would need a custom toast wrapper.
- Order-success confetti/checkmark celebration, drag-to-reorder inventory rows, button ripple/magnetic hover, input focus-glow — none started yet.

### How to apply this update
Copy the 6 files listed above into the matching paths in this repo (1:1 replace):
```
src/App.tsx
src/components/layout.tsx
...
```
Then `npm run dev` — no `npm install` needed, no new packages.

### Still outstanding from earlier notes
- **`api/` folder is still plain `.js`, not `.ts`** (`chat.js`, `notify.js`, `whatsapp.js`, `whatsapp-webhook.js`, `_lib/groqClient.js`, `_lib/supabaseServer.js`, `_lib/waClient.js`) — `place-order.js` was converted to `place-order.ts` as part of this update (below), so the JS→TS conversion is now partially done, not complete.

---

## 16. Recent Updates — Email (Resend) + Admin Registration with OTP (July 2026)

Adds customer order-confirmation emails and a self-service admin registration flow gated by a 6-digit code sent to **both** email and SMS. No Gmail SMTP — used **Resend** (the current international-standard transactional email API) instead, since Gmail SMTP isn't meant for production transactional mail (low daily limits, ToS risk of being flagged as bulk mail). Admin accounts still go through real **Supabase Auth** underneath — the OTP is an extra confirmation gate on top, not a replacement for it.

### New files
- **`api/_lib/emailClient.ts`** — Resend wrapper. `sendOrderConfirmationEmail()`, `sendAdminVerificationEmail()`. No-ops safely if `RESEND_API_KEY` isn't set (same pattern as `notify.js` for Twilio).
- **`api/_lib/smsClient.ts`** — Twilio SMS sender, extracted from `notify.js` so the OTP flow can text the *registering admin's own* phone (not just the fixed `ADMIN_PHONE_NUMBER`).
- **`api/_lib/otp.ts`** — 6-digit code generation + SHA-256 hashing (the raw code is never stored in the DB, only its hash).
- **`api/admin-register.ts`** — `POST {name, email, phone, password}`. Creates a Supabase Auth user with `email_confirm: false`, generates an OTP, stores its hash in `admin_verifications`, sends the code via email + SMS.
- **`api/admin-verify-code.ts`** — `POST {email, code}`. Checks the code (max 5 attempts, 10-minute expiry), then confirms the Supabase Auth user (`email_confirm: true`) so they can sign in.

### Changed files
- **`api/place-order.js` → `api/place-order.ts`** — converted to TypeScript; now accepts `email` from checkout and fires `sendOrderConfirmationEmail()` after a successful order (best-effort — email failure never blocks the order response).
- **`src/lib/utils.ts`** — `Order` interface gained an optional `email` field.
- **`src/screens/Store.tsx`** — checkout form now has a required Email field; the AI assistant's ordering flow (`CustomerAssistantWidget`) now also collects email as a 4th required field before it's allowed to place an order, so the manual and AI checkout paths stay consistent.
- **`src/App.tsx`** — `handlePlaceOrder` now forwards `email` to `/api/place-order` and stores it on the auto-created customer record.
- **`src/screens/Login.tsx`** — full rewrite. Admin tab now has 3 steps: **Sign in** (existing, unchanged behavior) → **Register** (name/phone/email/password → calls `/api/admin-register`) → **Verify** (6-digit code → calls `/api/admin-verify-code`, then auto signs in).
- **`package.json`** — added `@vercel/node` as a devDependency (type definitions for the new `.ts` API functions — Vercel bundles/runs `.ts` functions natively, no build config changes needed).

### Database — run this migration first
See `supabase-migration.sql` (included alongside this update): adds `orders.email` and a new `admin_verifications` table. Apply it in Supabase Dashboard → SQL Editor before deploying, or ask to have it applied directly via the connected Supabase tool.

### New environment variables (Vercel → Settings → Environment Variables)
| Variable | Used by | Notes |
|---|---|---|
| `RESEND_API_KEY` | `api/_lib/emailClient.ts` | Get free at resend.com (3,000 emails/month free tier). If unset, emails are silently skipped. |
| `EMAIL_FROM` | `api/_lib/emailClient.ts` | e.g. `AB OS <orders@yourdomain.com>`. Until you verify a domain on Resend, use the default `AB OS <onboarding@resend.dev>` (works immediately, no setup). |

Twilio vars (`TWILIO_ACCOUNT_SID`, `TWILIO_AUTH_TOKEN`, `TWILIO_PHONE_NUMBER`) are reused from the existing setup — no new SMS vars needed.

### How to apply this update
1. Run `supabase-migration.sql` in Supabase SQL Editor.
2. Copy the new/changed files listed above into matching paths.
3. Set `RESEND_API_KEY` (and optionally `EMAIL_FROM`) in Vercel env vars.
4. `npm install` (pulls in `@vercel/node` dev types), then `npm run dev` / redeploy.

### Not done in this pass
- Admin registration currently has no *approval* step beyond OTP — anyone who registers and verifies gets full admin access. If you want new admins to require owner approval too, that's a follow-up (e.g. an `is_approved` flag checked at login).
- No "resend code" button yet on the Verify screen — if the code expires, the user re-registers (which invalidates the old row via a fresh insert; old unverified rows are simply ignored, not deleted).

---

## 17. Payment Integration (Safepay) — Complete Reference (July 2026)

> Poori payment integration (Safepay) yahan ek jagah document hai — architecture, flow, files, database columns, environment variables, aur fix ki gayi bugs. Payment se related koi bhi kaam karne se pehle yeh section parhein.

**Provider:** Safepay (`getsafepay.com`) — Pakistan ka payment gateway (cards, wallets, etc.)
**SDK:** `@sfpy/node-core` (official Node SDK, server-side only)
**Currency:** PKR
**Mode:** Express Checkout (hosted checkout page — customer Safepay ke page par redirect hota hai)

### 17.1 High-Level Flow

```
Customer order place karta hai (Store.tsx / AI assistant / WhatsApp bot)
        │
        ▼
/api/place-order (service-role)
   ├─ Order Supabase mein save hota hai (payment_status: "unpaid")
   ├─ Stock validate + total server-side calculate hota hai
   ├─ Best-effort: Safepay checkout session banti hai (createSafepaySession)
   └─ Response mein order + checkoutUrl wapas jata hai
        │
        ▼
Customer ko checkoutUrl milta hai (Store UI / WhatsApp message / chat)
        │
        ▼
Customer Safepay ke hosted checkout page par payment karta hai
        │
        ▼
Safepay → /api/safepay-webhook ko POST karta hai (payment.succeeded / failed)
        │
        ▼
Webhook Safepay se dobara tracker verify karta hai (fetchSafepayTracker)
   └─ orders.payment_status update hota hai: "paid" / "failed" / "pending"
        │
        ▼
Customer ko wapas redirect kiya jata hai (redirectUrl / cancelUrl)
   └─ App.tsx URL query params (?payment=success&order_id=...) parse kar ke
      toast dikhata hai — asal payment_status hamesha webhook se aata hai,
      URL param sirf UI feedback ke liye hai (kabhi trust nahi kiya jata)
```

**Doosra path — Admin-triggered payment link:**
```
Admin OrdersView mein order select karta hai → "Send Payment Link"
        │
        ▼
/api/create-payment (order.id bhejta hai)
   ├─ Order ka asal total DB se fetch hota hai (client se aane wale amount par bharosa nahi)
   └─ Safepay checkout session ban ke checkoutUrl wapas aata hai
        │
        ▼
Admin link ko manually customer ko bhej sakta hai (WhatsApp/SMS/copy)
```
...
| File | Role |
|---|---|
| `api/_lib/safepayClient.ts` | Core Safepay wrapper — session create, checkout URL build, tracker fetch/verify. **Sirf server-side.** |
| `api/place-order.ts` | Customer order place hote hi best-effort payment session bhi bana deta hai |
| `api/create-payment.ts` | Admin/manual flow — existing order ke liye payment link banata hai |
| `api/safepay-webhook.ts` | Safepay se payment events receive karta hai, `orders.payment_status` update karta hai |
| `src/App.tsx` | `handlePlaceOrder()` — checkoutUrl ko caller tak pass karta hai; URL redirect params handle karta hai |
| `src/screens/Store.tsx` | Customer checkout — order place karne ke baad checkout link dikhata hai |
| `src/views/OrdersView.tsx` | Admin "Send Payment Link" flow — `PaymentStatusBadge` bhi yahin render hoti hai |
| `src/components/ui.tsx` | `PaymentStatusBadge` component (unpaid/pending/paid/failed states) |
| `src/config/app.config.ts` | `PAYMENT_STATUS_META` — badge colors/icons har status ke liye |
| `src/lib/utils.ts` | `Order.payment_status`, `Order.safepay_tracker` type definitions |

### 17.3 Database Columns (Supabase `orders` table)

| Column | Type | Values | Notes |
|---|---|---|---|
| `payment_status` | text | `unpaid` \| `pending` \| `paid` \| `failed` | Default `unpaid`. Sirf `/api/safepay-webhook` (verified) is column ko authoritative tareeqe se update karta hai. |
| `safepay_tracker` | text (nullable) | Safepay tracker token | Order ↔ Safepay session ko match karne ke liye — webhook isi se order dhoondta hai |

**Important:** `payment_status` order ke fulfillment `status` (pending/confirmed/delivered/cancelled) se **alag** hai — dono independent fields hain, do alag badges use hote hain UI mein.

### 17.4 Environment Variables

| Variable | Kahan use hota hai | Notes |
|---|---|---|
| `SAFEPAY_SECRET_KEY` | `api/_lib/safepayClient.ts` | Server-side sirf — kabhi frontend mein nahi jani chahiye |
| `SAFEPAY_PUBLIC_KEY` | `api/_lib/safepayClient.ts` | `merchant_api_key` (`sec_...` se shuru hota hai) |
| `SAFEPAY_ENV` | `api/_lib/safepayClient.ts` | `sandbox` (default) ya `production` |
| `PUBLIC_SITE_HOST` | `api/whatsapp-webhook.js` | WhatsApp bot ke payment links ke liye domain. Fallback: `abos-dashboard.vercel.app` |

`create-payment.ts` aur `place-order.ts` apna `origin` khud `req.headers.origin`/`req.headers.host` se nikal lete hain — unhein `PUBLIC_SITE_HOST` ki zaroorat nahi, sirf WhatsApp bot ko hai (jahan koi incoming HTTP request context nahi hota).

### 17.5 Bugs Fixed (July 19, 2026 debugging session)

1. **`axios` missing dependency** — `@sfpy/node-core` ko chahiye thi, `package.json` mein nahi thi. Poora `/api/place-order` crash karta tha (module-load-time failure, sirf payment nahi — ES module imports load-time par resolve hote hain, is liye `axios` na milne par poora serverless function process crash ho jata: `ERR_MODULE_NOT_FOUND`, exit code 1). ✅ Fixed — `package.json` mein `"axios": "^1.7.7"` add ki gayi (commit "Add axios dependency to package.json", deployment `dpl_HvN2DELWeNLBuvzzhgR1pxFAFvvd`).
2. **`new Safepay(...)` missing** — `api/_lib/safepayClient.ts` ke `getClient()` function mein `Safepay(secretKey, {...})` likha tha bina `new` ke. Official SDK docs ke mutabiq yeh ek class hai, `new` zaroori hai. Vercel build log mein TypeScript ne flag kiya (`TS2348: Value of type 'typeof Safepay' is not callable. Did you mean to include 'new'?`) — `vite build` type-check nahi karta is liye build fail nahi hui, lekin runtime par crash deti. ✅ Code fix ho chuka — `return new Safepay(...)`. **GitHub push + Vercel redeploy confirm karna baaki hai.**

### 17.6 Security Model

- **Amount kabhi client se nahi aata** — `create-payment.ts` aur `place-order.ts` dono order ka total seedha Supabase se fetch karte hain, phir Safepay session usi amount se banti hai.
- **Webhook payload par blind trust nahi** — `safepay-webhook.ts` webhook aane par bhi Safepay se seedha `fetchSafepayTracker()` call kar ke asal tracker state verify karta hai, tabhi `payment_status` update karta hai.
- **`SAFEPAY_SECRET_KEY` kabhi bundle mein nahi jati** — sirf `api/_lib/safepayClient.ts` (server-side) mein use hoti hai.

### 17.7 Known Limitations / Not Done Yet

- **PayPak provider** — abhi sirf Safepay hai; PayPak add karna hai ya nahi, yeh decision pending hai.
- **Live sandbox test abhi tak nahi hua** — Bug #2 ka fix deploy hone ke baad ek real order + real sandbox payment test karna zaroori hai.
- **Safepay webhook Safepay Dashboard mein register nahi hui** — `https://abos-dashboard.vercel.app/api/safepay-webhook` ko Safepay Dashboard → Developers → Webhooks mein add karna hoga, is ke bagair `payment_status` kabhi "paid" nahi banega chahe customer payment complete kar de.
- **`PUBLIC_SITE_HOST` Vercel env var** — abhi confirm nahi ke set hai ya nahi (fallback maujood hai is liye crash nahi hoga, lekin custom domain ke liye set karna behtar hai).
- **Retry/idempotency:** agar Safepay ek hi webhook event dobara bheje (jo webhooks mein normal hai), `safepay-webhook.ts` sirf `update` karta hai (idempotent hai — dobara chalne se koi masla nahi).

### 17.8 Testing Checklist (jab live test karna ho)

- [ ] Store se ek order place karein (valid Pakistani phone number, jaise `03XXXXXXXXX`)
- [ ] Confirm order Supabase `orders` table mein save hua (200 response, 500 nahi)
- [ ] Confirm `checkoutUrl` response mein aaya
- [ ] Checkout URL open karke Safepay sandbox par test payment complete karein
- [ ] Confirm `/api/safepay-webhook` fire hua aur `orders.payment_status` "paid" ho gaya (Supabase Table Editor se check karein)
